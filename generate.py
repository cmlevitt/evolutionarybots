...
def Create_Robot():
    ...
    pyrosim.Send_Joint(name = "Torso_BackLeg" , parent= "Torso" , child = "BackLeg" , type = "revolute", position = [0,0.5,1.5])
...
'''
    ...
    pyrosim.Send_Cube(name="Link5", pos=[0,0,-1.0] , size=[length, width, height])
    pyrosim.Send_Joint(name = "Link5_Link6" , parent= "Link5" , child = "Link6" , type = "revolute", position = [0,0,-1.0])
    pyrosim.Send_Cube(name="Link6", pos=[0,0,-1.0] , size=[length, width, height])
    pyrosim.End()
'''
Create_World()
Create_Robot()